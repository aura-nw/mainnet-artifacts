...
DENOM=uaura
MAXBOND=7500000000 # 7500 aura
MINCOMMISSION="0.05"

GENTX_SUBMISSION_START=$(date -u -d '2023-03-09T14:00:00.000Z' +'%s')
GENTX_SUBMISSION_DEADLINE=$(date -u -d '2023-03-16T08:00:00.000Z' +'%s')
...
if [ $LEN_GENTX -eq 0 ]; then
    ...
    if [ $amount -gt $MAXBOND ]; then
        echo "bonded amount is larger than 7500aura: $amt > $MAXBOND"
        exit 1
    fi
    ...
    # curl -s https://raw.githubusercontent.com/aura-nw/mainnet-artifacts/main/xstaxy-1/pre-genesis.json >$AURA_HOME/config/genesis.json
    ...
    ./build/aurad add-genesis-account $GENACC $amount$DENOM --home $AURA_HOME
